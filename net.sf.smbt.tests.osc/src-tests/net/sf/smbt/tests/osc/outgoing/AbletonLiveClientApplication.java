...
public class AbletonLiveClientApplication implements IApplication {
	private AbletonLive liveApp;
	
	public AbletonLiveClientApplication() {
			...
			new AbstractQxEventHandlerImpl() {
				public void handleQxEvent(net.sf.xqz.model.engine.Event event) {
					if (event.getKind().equals(EVENT_KIND.RX_DONE)) {
						handleAbletonLiveRcvCmd(event.getCmd());
					}
				}
			}
		);
	...
	private void handleAbletonLiveRcvCmd(Cmd cmd) {
		System.out.println("Ableton !!!!!");
		if (cmd instanceof AbletonLiveSndCmd) {
			if (cmd instanceof LiveClipPosition) {
				AbletonLiveControllerUtils.INSTANCE.handleLiveClipPosition(liveApp, (LiveClipPosition) cmd);
			}
