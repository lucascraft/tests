...
package net.sf.smbt.tests.osc.ableton;
...
import net.sf.smbt.osc.ableton.utils.AbletonLiveUtils;
...
public class AbletonLiveClientGUIControlApplication implements IApplication {
	
	private AbletonLive liveApp;
	
	public AbletonLiveClientGUIControlApplication() {
		liveApp = AbletonLiveControllerUtils.INSTANCE.createDefaultAbletonLiveSession();
	}

	private void handleAbletonLiveRcvCmd(Cmd cmd, CmdPipe oscAbletonEngine) {
		AbletonLiveControllerUtils.INSTANCE.handleLiveControlSynchro(cmd, liveApp, oscAbletonEngine);
	}
	@Override
	public Object start(IApplicationContext context) throws Exception {
		
		final CmdPipe oscAbletonEngine = QuanticMojo.INSTANCE.openUdpPipe(
			"oscableton", 
			"oscAbleton0", 
			"localhost:9000", 
			new int[] { 9001 }
		);
		oscAbletonEngine.addQxEventHandler(
			new AbstractQxEventHandlerImpl() {
				public void handleQxEvent(net.sf.xqz.model.engine.Event event) {
					if (event.getKind().equals(EVENT_KIND.RX_DONE)) {
						handleAbletonLiveRcvCmd(event.getCmd(), oscAbletonEngine);
					}
				}
			}
		);
		liveApp.getEngines().add(oscAbletonEngine);
		
		final CmdPipe tuioEngine = QuanticMojo.INSTANCE.openUdpPipe(
			"tuio1", 
			"tuioSimulator", 
			"localhost:3334", 
			new int[] { 3333 }
		...
		tuioEngine.addQxEventHandler(
			new AbstractQxEventHandlerImpl() {
				public void handleQxEvent(net.sf.xqz.model.engine.Event event) {
					if (event.getKind().equals(EVENT_KIND.RX_DONE)) {
						handleTuioRcvCmd(oscAbletonEngine, event.getCmd());
					}
				}
			}
		);

		final CmdPipe oscWiimoteEngine = QuanticMojo.INSTANCE.openUdpPipe(
			"oscwiimote", 
			"oscWiimote0", 
			"", 
			new int[] { 1234 }
		...
		oscWiimoteEngine.addQxEventHandler(
			new AbstractQxEventHandlerImpl() {
				public void handleQxEvent(net.sf.xqz.model.engine.Event event) {
					if (event.getKind().equals(EVENT_KIND.RX_DONE)) {
						handleWiimoteRcvCmd(oscAbletonEngine, event.getCmd());
					}
				}
			}
		);
		
		AbstractTTLJob job = new AutoPlaySceneJob(oscAbletonEngine, 5);
		job.schedule();
